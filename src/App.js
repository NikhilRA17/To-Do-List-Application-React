...
function App() {
  ...
  return (
    <div className="App"  >
      <h1 className={"display-1"} style={{ marginTop: '20px', fontSize: '50px' }}>TO-DO LIST</h1>
      <div className={"input-group flex-nowrap"} style={{ width: '80%', margin: 'auto' }}>
        <input type="text"  onChange={handChange} className={"form-control input-group-text"} id="addon-wrapping" placeholder="" aria-label="Username" aria-describedby="addon-wrapping" style={{ width: '200px', marginTop: '15px' }} />
      </div>

      <div>
         <input type={'date'} onChange={dateChange} style={{marginTop:'25px'}} id={'inputDate'}></input>
      </div>
      
      <button type="button" onClick={handClick} className={"btn btn-outline-success"} style={{ width: '100px', marginTop: '25px' }}>Click</button>

      <ul id={'ul'} style={{listStyle: 'none',  textAlign:'start', padding:'50px'}} className={"list-group"}></ul>
